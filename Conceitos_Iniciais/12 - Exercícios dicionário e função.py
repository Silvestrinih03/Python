...
# Desenvolver a seguintes funções:

# 1) Dado o código, retornar o nome da disciplina.
# FUNÇÃO PARA DADO O CÓDIGO DE UMA DISCIPLINA RETORNAR O NOME DA MESMA
# INPUT: CÓDIGO DA DISCIPLINA e DATASET DSDISCIPLINAS
# RETURN: NOME DA DISCIPLINA
def nome_disciplina(codigodisciplina,dsdisciplina):
    for lin in dsdisciplina:
        cd=lin["CODIGODISCIPLINA"]
        nd=lin["DISCIPLINA"]
        if codigodisciplina==cd:
            return nd
    return None

codigo=input("DIGITE O CÓDIGO DA DISCIPLINA: ")
disciplina=nome_disciplina(codigo,DSdisciplinas)
print(f"O código {codigo} pertence a disciplina {disciplina}")


# 2) Dado o nome de um aluno, retornar um vetor com o código das disciplinas que está cursando.
# FUNÇÃO PARA DADO O NOME DO ALUNO RETORNAR O CODIGO DAS DISCIPLINAS QUE ELE ESTÁ CURSANDO
# INPUT: NOME DO ALUNO e DATASET DSDISCIPLINAS
# RETURN: CODIGO DAS DISCIPLINAS
def disciplinas_aluno(nomealuno,dsaluno):
    for lin in dsaluno:
        nm=lin["NOME"]
        codigo_ds=lin["DISCIPLINAS"]
        if nomealuno==nm:
            return codigo_ds
    return None

nome=input("DIGITE O NOME DO ALUNO: ").upper()
codigodisciplinas=disciplinas_aluno(nome,DSalunos)
print(f"O aluno {nome} cursa as disciplinas {codigodisciplinas}")


# 3) Dado o nome de um aluno, retornar um vetor com o nome das disciplinas que está cursando.
# FUNÇÃO PARA DADO O NOME DO ALUNO RETORNAR O NOME DAS DISCIPLINAS QUE ELE ESTÁ CURSANDO
# INPUT: NOME DO ALUNO e DATASET DSDISCIPLINAS
# RETURN: NOME DAS DISCIPLINAS
def disciplinas_aluno(nomealuno,dsaluno):
    for lin in dsaluno:
        nm=lin["NOME"]
        codigo_ds=lin["DISCIPLINAS"]
...
nome=input("DIGITE O NOME DO ALUNO: ").upper()
codigodisciplinas=disciplinas_aluno(nome,DSalunos)
print(f"O Aluno {nome} cursa as disciplinas:")
for i in codigodisciplinas:
    disciplina=nome_disciplina(i,DSdisciplinas)
    print(disciplina)

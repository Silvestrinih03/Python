...
# Listar os cursos cadastrados - retornar o número de cadastros ----> OK
def listar_cursos(dscursos):
    cursos = []
    for lin in dscursos:
        cd = lin['CODIGO']
        nc = lin['NOME']
        dr = lin["DURACAO"]
        cursos.append(nc)
    return len(cursos), cursos

numerocursos, cursos = listar_cursos(DScurso)
print(f"A PUC tem {numerocursos} cursos cadastrados!")
for lin in cursos:
    print(lin)

# Dado um código de curso, retornar o nome do curso e duração em anos ----> OK
def nomecurso(codcurso, dscurso):
    for lin in dscurso:
        cd = lin['CODIGO']
        nc = lin['NOME']
        dr = lin["DURACAO"]
        if codcurso == cd:
            return f'pertence ao curso "{nc.lower()}" e tem duração de {dr} anos'
    return "não está cadastrado no sistema!"
        
codCurso = input("\nDigite o código do curso: ")
retorna = nomecurso(codCurso, DScurso)
print(f"O código {codCurso} {retorna}")

# Dado o nome de um curso, retornar o código do curso e duração em anos ----> OK
def codigocurso(nomecurso, dscurso):
    for lin in dscurso:
        cd = lin['CODIGO']
        nc = lin['NOME']
        dr = lin["DURACAO"]
        if nomecurso == nc:
            return f'O código do curso {nomeCurso.lower()} é {cd} e tem duração de {dr} anos'
    return f"O curso {nomeCurso} não está cadastrado no sistema!"

nomeCurso = input("\nDigite o nome do curso: ").upper()
retorna = codigocurso(nomeCurso, DScurso)
print(retorna)

# Dado um código de curso, retornar um vetor com o nome de todas das disciplinas cadastradas para este curso ----> OK
def disciplinasCurso(codCurso, dsdisciplinas):
    listadisciplina = []
    for lin in dsdisciplinas:
        codcurso = lin["CODIGOCURSO"]
        nomedisciplina = lin["DISCIPLINA"]
        if codCurso == codcurso:
            listadisciplina.append(nomedisciplina)
    if listadisciplina != []:
        return f"Vetor das disciplinas cadastradas para esse curso:\n{listadisciplina}"
    return f"O código {codCurso} não está cadastrado no sistema!"

codCurso = input("\nDigite o código do curso: ")
retorna = disciplinasCurso(codCurso, DSdisciplinas)
print(retorna)

# Dado o nome de um curso, retornar um vetor com o nome de todas das disciplinas cadastradas para este curso ----> OK
def buscarcodigocurso(nomeCurso, dscurso):
    for lin in dscurso:
        cd = lin['CODIGO']
        nc = lin['NOME']
        if nomeCurso == nc:
            return cd
    return "Esse curso não está cadastrado!"

nomeCurso = input("\nDigite o nome do curso: ").upper()
codCurso = buscarcodigocurso(nomeCurso, DScurso)
retorna = disciplinasCurso(codCurso, DSdisciplinas)
print(retorna)

# Dado um código de curso, retornar a carga horária total de todas das disciplinas cadastradas para este curso ----> OK
def cargahorariaCurso(codCurso, dsdisciplinas):
    soma = 0
    for lin in dsdisciplinas:
        codcurso = lin["CODIGOCURSO"]
        horaaula = lin["HORASAULAS"]
        if codCurso == codcurso:
            soma+=horaaula
    if soma > 0:
        return f"A carga horária total de todas as diciplinas cadastradas para esse curso é de {soma} horas"
    return f"O código {codCurso} não está cadastrado no sistema!"

codCurso = input("\nDigite o código do curso: ")
retorna = cargahorariaCurso(codCurso, DSdisciplinas)
print(retorna)

# Dado o nome de um curso, retornar a carga horária total de todas das disciplinas cadastradas para este curso ----> OK
def buscarcodigocurso(nomeCurso, dscurso):
    for lin in dscurso:
        cd = lin['CODIGO']
        nc = lin['NOME']
...
# Dado o código de um curso, retornar um vetor contendo o código, o nome e a carga horária das disciplinas ----> OK
def disciplinasCurso(codCurso, dsdisciplinas):
    listadisciplina = []
    for lin in dsdisciplinas:
        codcurso = lin["CODIGOCURSO"]
        coddisciplina = lin["CODIGODISCIPLINA"]
        nomedisciplina = lin["DISCIPLINA"]
        cargahoraria = lin["HORASAULAS"]
        if codCurso == codcurso:
            listadisciplina.append({coddisciplina: [nomedisciplina, cargahoraria]})
    if listadisciplina != []:
        return f"Vetor das disciplinas cadastradas para esse curso:\n{listadisciplina}"
    return f"O código {codCurso} não está cadastrado no sistema!"
